...
exports.default = function (_ref) {
  var deleted = _ref.deleted,
      ...
      renamed = _ref.renamed,
      added = _ref.added,
      _ref$sortKeys = _ref.sortKeys,
      sortKeys = _ref$sortKeys === undefined ? true : _ref$sortKeys;

  if (!(deleted.length || added.length || untranslated.length || renamed.length)) {
    ...
    if (renamed.length) {
      var _items = sortKeys ? renamed.sort(_compareByKey2.default) : renamed;
      (0, _printer.subheader)('Renamed keys:');
      _items.forEach(function (_ref3) {
        var key = _ref3.key,
            from = _ref3.from,
            message = _ref3.message;
        return console.log('  ' + (0, _chalk.red)(from) + ' -> ' + (0, _chalk.green)(key) + ': ' + (0, _chalk.cyan)(message));
      });
      (0, _printer.newLine)();
    }

    if (untranslated.length) {
      var _items2 = sortKeys ? untranslated.sort(_compareByKey2.default) : untranslated;
    ...
    if (added.length) {
      var _items3 = sortKeys ? added.sort(_compareByKey2.default) : added;
